...
from ..data.queries.sf_inference_archive_data import sf_inference_archive_data
from ..data.queries.sf_inference_latest_date import sf_inference_latest_date
from ..data.schemas import inference_s3_output_columns, inference_sf_output_columns
from ..generic.output_data_pipeline import OutputDataPipeline
from ..data.queries.sf_inference_latest_date import sf_inference_latest_date
from ..data.queries.sf_inference_archive_data import sf_inference_archive_data
from ..data.schemas import inference_s3_output_columns, inference_sf_output_columns
        ...
        for s3_key, modified_dt in s3_files:
            ...
            if check_csv == "csv.out":
                s3_path = (
                    ...
                    + f"{self.env_config.get('s3_bucket')}/"
                    + f"{s3_key}"
                )
