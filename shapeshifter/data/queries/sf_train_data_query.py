"""
Snowflake query to fetch data for training data pipeline
"""


def sf_train_so_line_data_query(
    start_date: str,
    end_date: str,
) -> str:
    """
    Function to retrieve the query to fetch the actuals of SO schedule line data from snowflake.
    ...
    query = f"""
        ...
        AVG_PER_SILHOUETTE AS (
            ...
            INNER JOIN (SELECT SO_DOC_HDR_NBR, SILH_DESC, SO_HDR_CRT_DATE FROM EMEA_DA_CONS_PROD.MPO.ORDERBOOK_FOUNDATION_HISTORY_V GROUP BY SO_DOC_HDR_NBR, SILH_DESC, SO_HDR_CRT_DATE) OBF
                ON DD.SALES_ORDER_NUMBER = OBF.SO_DOC_HDR_NBR
            WHERE SO_HDR_CRT_DATE >= '{start_date}'
            GROUP BY 
                OBF.SILH_DESC,
                OBF.SO_HDR_CRT_DATE
        ),
        AVG_PER_CUSTOMER AS (
            ...
            INNER JOIN EMEA_DA_CONS_PROD.DCDASH.VIEW_CONSUMABLE_OUTBOUND_DELIVERY_DOC_DETAIL DD
                ...
                AND CI.PRODUCT_CODE = DD.PRODUCT_CODE
                AND CI.SIZE_CODE = DD.SIZE_CODE 
            RIGHT JOIN (SELECT SO_DOC_HDR_NBR, SO_HDR_CRT_DATE, CUST_SHIP_TO_CD FROM EMEA_DA_CONS_PROD.MPO.ORDERBOOK_FOUNDATION_HISTORY_V GROUP BY SO_DOC_HDR_NBR, SO_HDR_CRT_DATE, CUST_SHIP_TO_CD) OBF
                ON DD.SALES_ORDER_NUMBER = OBF.SO_DOC_HDR_NBR
            WHERE OBF.CUST_SHIP_TO_CD IS NOT NULL
            AND SO_HDR_CRT_DATE >= '{start_date}'
        ...
        OBF_LAST_STATUS AS (
            SELECT 
                ...
                OBF.SO_LN_ITM_NBR,
                OBF.SO_SCHED_LN_NBR,
                MAX(OBF.SNAPSHOT_DATE) AS LAST_SNAPSHOT
            FROM EMEA_DA_CONS_PROD.MPO.ORDERBOOK_FOUNDATION_HISTORY_V OBF
            WHERE OBF.SO_HDR_CRT_DATE >= '{start_date}'
            GROUP BY 
                OBF.SO_DOC_HDR_NBR,
                OBF.SO_LN_ITM_NBR,
                OBF.SO_SCHED_LN_NBR
        ...
        BASE AS (
            SELECT
                ...
                DD.SHIP_TO_CUSTOMER_NUMBER,
                -- we use actual delivered quantities for training, but sosl quantities for inference
                CI.ACTUAL_DELIVERY_UNITS_QUANTITY AS ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY,
                ...
                C.ACTUAL_DELIVERY_UNITS_QUANTITY AS TOTAL_CARTON_QUANTITY,
                SO_HDR_CRT_DATE AS SALES_ORDER_HEADER_DOCUMENT_DATE
            FROM EMEA_DA_CONS_PROD.MPO.ORDERBOOK_FOUNDATION_HISTORY_V OBF
            INNER JOIN OBF_LAST_STATUS
                ON OBF.SO_DOC_HDR_NBR = OBF_LAST_STATUS.SO_DOC_HDR_NBR
                AND OBF.SO_LN_ITM_NBR = OBF_LAST_STATUS.SO_LN_ITM_NBR
                AND OBF.SO_SCHED_LN_NBR = OBF_LAST_STATUS.SO_SCHED_LN_NBR
                AND OBF.SNAPSHOT_DATE = OBF_LAST_STATUS.LAST_SNAPSHOT
            INNER JOIN EMEA_DA_CONS_PROD.DCDASH.VIEW_CONSUMABLE_OUTBOUND_DELIVERY_DOC_DETAIL DD
                ON DD.SALES_ORDER_NUMBER = OBF.SO_DOC_HDR_NBR
                AND DD.PRODUCT_CODE = OBF.PROD_CD
                AND DD.SIZE_CODE = OBF.SZ_DESC
            INNER JOIN EMEA_DA_FDN_PROD.DCDASH.CONSUMABLE_CARTON_ITEM_FULL CI
                ON CI.OUTBOUND_DELIVERY_HEADER_NUMBER = DD.OUTBOUND_DELIVERY_HEADER_NUMBER
                AND CI.PRODUCT_CODE = DD.PRODUCT_CODE
                AND CI.SIZE_CODE = DD.SIZE_CODE
            INNER JOIN EMEA_DA_FDN_PROD.DCDASH.CONSUMABLE_CARTON_FULL C
                ON C.CARTON_NUMBER = CI.CARTON_NUMBER
                AND C.DIGITAL_FLAG = 'N'
            ...
            WHERE   OBF.ACTVIND='Y'
                ...
                AND OBF.SO_HDR_CRT_DATE BETWEEN '{start_date}' AND '{end_date}'
        )
        SELECT 
            ...
            SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY) AS SOSL_TOTAL_QTY,
            ...
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER, BASE.SALES_ORDER_ITEM_NUMBER) AS SOI_TOTAL_QUANTITY,
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER) AS SOH_TOTAL_QUANTITY,
            SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY) / MAX(BASE.FULL_CASE_QUANTITY_MAX) AS SOSL_FULL_CASE_MAX_EQUIVALENT,
            SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY) / MAX(BASE.FULL_CASE_QUANTITY_MODE) AS SOSL_FULL_CASE_MODE_EQUIVALENT,
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER, BASE.SALES_ORDER_ITEM_NUMBER) / MAX(BASE.FULL_CASE_QUANTITY_MAX) AS SOI_FULL_CASE_MAX_EQUIVALENT,
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER, BASE.SALES_ORDER_ITEM_NUMBER) / MAX(BASE.FULL_CASE_QUANTITY_MODE) AS SOI_FULL_CASE_MODE_EQUIVALENT,
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER) / MAX(BASE.FULL_CASE_QUANTITY_MAX) AS SOH_FULL_CASE_MAX_EQUIVALENT,
            SUM(SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY)) OVER (PARTITION BY BASE.SALES_ORDER_HEADER_NUMBER) / MAX(BASE.FULL_CASE_QUANTITY_MODE) AS SOH_FULL_CASE_MODE_EQUIVALENT,
            ...
            -- COUNT(DISTINCT BASE.CARTON_NUMBER) AS NBR_CARTONS,
            SUM(BASE.ACTUAL_SHIPPED_ORDER_SCHEDULE_LINE_QTY) / MAX(BASE.TOTAL_CARTON_QUANTITY) AS NBR_CARTONS_RATIO,
